<template>
    ...
    <el-table :data="tableData" border style="width: 100%;margin-top: -70px;">
        ...
        <el-table-column prop="levels" label="级别" width="120" />
    ...
    setup() {
        ...
        const addAction = (form) => {
            ...
            if (form.StudentId == '' || form.levels == '' || !errorvalue) {
                ...
                apiService.insertAction("reward", form).then(
                    (response) => {
                        if (!response.data.ok) {
                            alert("输入错误或服务端错误")
                        } else {
        ...
        const getall = async () => {
            alldata.value = [];
            await apiService.getall("reward").then(
                (response) => {
                    // 获取学生信息的 API 方法
                    if (response.data.ok) {
        ...
        const ChangeSearch = async (searchCriteria) => {
            await apiService.searchAction("reward", searchCriteria.type, searchCriteria.value).then(
                (response) => {
                    if (response.data.ok) {
                        let res = response.data.info.data
                        alldata.value = res.map((item, index) => ({
        ...
        const deleteEntry = (row) => {
            apiService.deleteAction("reward", row.id).then(
                (response) => {
                    if (response.data.ok) {
                        getall();
                    }
        ...
        const saveChanges = async (row) => {
            ...
            for (let i = 0; i < inputs.length; i++) {
                ...
                if (res != data[i + 1]) {
                    await (apiService.updateAction('reward', data[0], column[i + 1], res).
                        then((response) => {
                            if (response.data.ok) {
                            } else {
                                // TODO 异常处理...
