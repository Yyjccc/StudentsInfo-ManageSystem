<template>
    ...
    <el-table :data="tableData" border style="width: 100%;margin-top: -70px;">
        ...
        <el-table-column prop="changeId" label="变更代码" width="120" />
        <el-table-column prop="recTime" label="记录时间" width="200" />
    ...
    <div>
        <el-dialog v-model="dialogFormVisible" title="添加学籍变更信息" :append-to-body="true">
            <el-form :model="form">
                ...
                <el-form-item label="变更代码" :label-width="formLabelWidth">
                    <el-input v-model="form.changeId" autocomplete="off" />
                </el-form-item>
...
export default {
    ...
    setup() {
        ...
        const addAction = (form) => {
            ...
            //校验数据
            if (form.StudentId == '' || form.changeId == '' || !errorvalue) {
        ...
        const typemy = ref("学生学籍变更表")

        //编辑
        const editMode = ref([]);
        const column = ['index', 'id', 'StudentId', 'changeId', 'recTime', 'description']
        ...
        const getall = async () => {
            alldata.value = [];
            await apiService.getall("change").then(
                (response) => {
                    // 获取学生信息的 API 方法
                    if (response.data.ok) {
        ...
        const ChangeSearch = async (searchCriteria) => {
            await apiService.searchAction("change", searchCriteria.type, searchCriteria.value).then(
                (response) => {
                    if (response.data.ok) {
                        let res = response.data.info.data
                        alldata.value = res.map((item, index) => ({
        ...
        const deleteEntry = (row) => {
            apiService.deleteAction("change", row.id).then(
                (response) => {
                    if (response.data.ok) {
                        getall();
                    }
        ...
        const saveChanges = async (row) => {
            ...
            for (let i = 0; i < inputs.length; i++) {
                ...
                if (res != data[i + 1]) {
                    await (apiService.updateAction('change', data[1], column[i + 1], res).
                        then((response) => {
                            if (response.data.ok) {
                            } else {
                                // TODO 异常处理...
